...
export async function generateImageToPdf(input: ImageToPdfInput): Promise<Uint8Array> {
  ...
  // Build template: Page 1 = header + first image, Page 2+ = image only
  ...
  input.pages.forEach((page, idx) => {
    ...
    if (idx === 0) {
      // First page: header block (like quotation) + image
      schemas.push(
        ...
        { name: 'clientInfoBox', type: 'rectangle', position: { x: 15, y: 64 }, width: 180, height: 28, color: '#E5E5E5', radius: 4 },
        ...
        { name: 'dateLabel', type: 'text', position: { x: 18, y: 79 }, width: 25, height: 5, fontSize: 9, fontColor: '#000', fontName: 'Helvetica-Bold', alignment: 'left' },
        { name: 'dateValue', type: 'text', position: { x: 47, y: 79 }, width: 55, height: 5, fontSize: 9, fontColor: '#000', fontName: 'Helvetica', alignment: 'left' }
      )
      inputs.logo = input.companyLogo || ''
    ...
    // Image: centered, below header (page 1) or full page (page 2+)
    const imgTop = idx === 0 ? 100 : 15
    const imgHeight = idx === 0 ? 150 : PAGE_HEIGHT - 55
    const imgWidth = 180
    ...
    const nameTop = idx === 0 ? 255 : PAGE_HEIGHT - 35
    schemas.push({
      name: `designName${idx}`,
      type: 'text',
      position: { x: 15, y: nameTop },
      width: 180,
      height: 10,
