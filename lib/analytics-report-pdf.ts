...
export async function generateAnalyticsReportPDF(
  ...
  if (!hasPage2Content) {
    page1Input.summary = `Analytics Report | ${params.comprehensiveChartData.length} data points | Generated by Cabinet Master Analytics`
    page1Input.preparedBy = 'Prepared by: ____________'
    page1Input.approvedBy = 'Approved by: ____________'
    page1Input.pageNumber = 'Page 1 of 1'
  }

  schemas.push(page1Schema)
  inputs.push(page1Input)

  // ---- PAGE 2: Segmentation + AI Insights (only if has content) ----
  if (hasPage2Content) {
