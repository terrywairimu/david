...
const SalesOrderModal: React.FC<SalesOrderModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      fetchStockItems()
      if (mode === "create") {
        generateSalesOrderNumber().then(setOrderNumber)
        resetForm()
  ...
  const loadSalesOrderData = () => {
    ...
    setLabourPercentage(Number(salesOrder.labour_percentage) || 30)
    ...
    setCabinetLabourPercentage(Number(salesOrder.cabinet_labour_percentage) || 30)
  ...
  const generatePDF = async () => {
    try {
      ...
      const { template, inputs } = await generateSalesOrderPDF(salesOrderData);
      ...
      link.href = url;
      link.download = `salesOrder-${orderNumber}.pdf`;
  ...
  const handleShare = async () => {
    try {
      ...
      if (canShareFile && pdfBlob) {
        const file = new File([pdfBlob], `salesOrder-${orderNumber}.pdf`, { type: 'application/pdf' })
  ...
  const generatePDFForViewing = async () => {
    ...
    try {
      ...
      const { generate } = await import('@pdfme/generator');
      const { text, rectangle, line, image } = await import('@pdfme/schemas');
      const { generateSalesOrderPDF, imageToBase64 } = await import('@/lib/pdf-template');
      ...
      sectionOrder.forEach((category) => {
        ...
        if (category === 'worktop' && salesOrder.worktop_labor_qty && salesOrder.worktop_labor_unit_price) {
          currentSectionTotal += salesOrder.worktop_labor_qty * salesOrder.worktop_labor_unit_price;
        }
      ...
      // Prepare salesOrder data (same as working download PDF)
      const { template, inputs } = await generateSalesOrderPDF({
        companyName: "CABINET MASTER STYLES & FINISHES",
        companyLocation: "Location: Ruiru Eastern By-Pass",
        companyPhone: "Tel: +254729554475",
        companyEmail: "Email: cabinetmasterstyles@gmail.com",
  ...
  const checkAndAutoProgress = async (salesOrderData: any) => {
    try {
      // Get the original quotation to check payment status
      if (salesOrderData.original_quotation_number) {
        const { data: quotation, error } = await supabase
          ...
          `)
          .eq("quotation_number", salesOrderData.original_quotation_number)
          .single()

        if (error) throw error

        if (quotation && quotation.quotation_payments) {
          const totalPaid = quotation.quotation_payments
          ...
          // Auto-progress based on payment percentage
          if (paymentPercentage >= 100 && quotation.status !== "converted_to_cash_sale") {
            // 100% paid - convert to cash sale
            await supabase
              .from("quotations")
              .update({ status: "converted_to_cash_sale" })
              .eq("id", quotation.id)
            
            toast.success("Quotation automatically converted to Cash Sale (100% payment received)")
          } else if (paymentPercentage >= 75 && quotation.status !== "converted_to_invoice") {
            // 75% paid - convert to invoice
            await supabase
              .from("quotations")
              .update({ status: "converted_to_invoice" })
              .eq("id", quotation.id)
            
            toast.success("Quotation automatically converted to Invoice (75% payment received)")
          } else if (paymentPercentage > 0 && quotation.status !== "converted_to_sales_order") {
            // Any payment - convert to sales order
            await supabase
              .from("quotations")
              .update({ status: "converted_to_sales_order" })
              .eq("id", quotation.id)
            
            toast.success("Quotation automatically converted to Sales Order (payment received)")
          }
        }
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Add Labour Charge to accessories, appliances, wardrobes, tvunit when include labour is on (for action column PDF)
    ...
    // Confirm salesOrder number if it's a new salesOrder
    // Removed localStorage logic, so this block is effectively removed.
    // The generateSalesOrderNumber function now handles the number generation.

    await onSave(salesOrderData)
    
    // Check for automated progression
    await checkAndAutoProgress(salesOrderData)
    
    onClose()
    } catch (error) {

      toast.error("Failed to save salesOrder")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <>
      <div className="modal fade show d-block salesOrder-modal" style={{ backgroundColor: "rgba(0,0,0,0.5)" }}>
        <div 
          ...
          <div className="modal-header border-0" style={{ padding: "24px 32px 16px" }}>
            <div className="d-flex align-items-center">
              ...
              <div>
                ...
                {mode !== "view" && (
                  <p className="mb-0 text-white small">Create a detailed salesOrder for your client</p>
                )}
              </div>
            </div>
            <button
                ...
                <div className="card-body p-4">
                  ...
                  <div className="mb-3">
                    ...
                    {cabinetItems.map((item, index) => (
                      <div key={item.id} className="d-flex flex-column flex-md-row align-items-start align-items-md-center mb-3 mb-md-2 salesOrder-item-row worktop-row">
                  ...
                  {includeWardrobes && (
                    <div className="mb-3">
                      ...
                      {wardrobesItems.map((item, index) => (
                        <div key={item.id} className="d-flex flex-md-row align-items-start align-items-md-center mb-3 mb-md-2 salesOrder-item-row worktop-row" style={{ flexDirection: 'row' }}>
