...
const InvoiceModal: React.FC<InvoiceModalProps> = ({
  ...
  const [formData, setFormData] = useState<Invoice>({
    ...
    status: "pending",
    notes: "",
    terms_conditions: "1. Payment due within 30 days of invoice date.\n2. Late payment may incur additional charges.\n3. All disputes must be raised within 7 days of invoice date.\n4. Goods remain the property of Cabinet Master until full payment is received.",
    items: [],
    payments: []
  })
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (invoice) {
        ...
        // Set due date to 30 days from creation if not set
        if (!invoice.due_date) {
          const dueDate = new Date(invoice.date_created)
          dueDate.setDate(dueDate.getDate() + 30)
          setFormData(prev => ({ ...prev, due_date: dueDate.toISOString().split('T')[0] }))
        }
        
        // Separate items by category
        const cabinet = invoice.items?.filter(item => item.category === "cabinet") || []
        const worktop = invoice.items?.filter(item => item.category === "worktop") || []
        const accessories = invoice.items?.filter(item => item.category === "accessories") || []
  ...
  const updateItem = (category: "cabinet" | "worktop" | "accessories", index: number, field: keyof InvoiceItem, value: any) => {
  ...
  const calculateTotals = () => {
    ...
    const balanceAmount = grandTotal - formData.paid_amount
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">
              {mode === "view" ? "View" : mode === "edit" ? "Edit" : "New"} Invoice
            </h5>
            <button
              type="button"
              className="btn-close"
                  ...
                  <table className="quotation-table">
                    ...
                    <tfoot>
                      <tr>
                        ...
                        <td colSpan={2}><strong>KES {formData.paid_amount.toFixed(2)}</strong></td>
                      </tr>
                    </tfoot>
                  </table>
                </>
              ...
              <table className="quotation-table">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td><strong>KES {formData.paid_amount.toFixed(2)}</strong></td>
                  </tr>
                  <tr>
