...
const generateCashSaleNumber = async () => {
  try {
    const now = new Date()
    const year = now.getFullYear().toString().slice(-2)
    const month = (now.getMonth() + 1).toString().padStart(2, '0')
    // Query Supabase for the latest cashSale number for this year/month
    const { data, error } = await supabase
      .from("cashSales")
      .select("cashSale_number")
      .ilike("cashSale_number", `QT${year}${month}%`)
      .order("cashSale_number", { ascending: false })
      .limit(1)
    if (error) throw error
    let nextNumber = 1
    if (data && data.length > 0) {
      const match = data[0].cashSale_number.match(/QT\d{4}(\d{3})/)
      if (match) {
...
const CashSaleModal: React.FC<CashSaleModalProps> = ({
  ...
  const [appliancesItems, setAppliancesItems] = useState<CashSaleItem[]>([])
  const [wardrobesItems, setWardrobesItems] = useState<CashSaleItem[]>([])
  const [tvUnitItems, setTvUnitItems] = useState<CashSaleItem[]>([])
  ...
  const fetchPaymentInfo = async () => {
    if (!cashSale?.cashSale_number) return;
    
    try {
      // First, check all payments for this cashSale regardless of status - check both fields
      const { data: allPayments } = await supabase
        .from("payments")
        .select("*")
        .or(`cashSale_number.eq.${cashSale.cashSale_number},paid_to.eq.${cashSale.cashSale_number}`)

      // Then get only completed payments - check both cashSale_number and paid_to fields
      ...
      const paymentPercentageValue = cashSale.grand_total > 0 ? (totalPaidAmount / cashSale.grand_total) * 100 : 0
      
      // Enhanced debug logging for payment detection
      console.log('Payment Info Debug:', {
        cashSale_number: cashSale.cashSale_number,
        cashSale_status: cashSale.status,
      ...
      // Also check if there are payments with different cashSale_number formats
      const { data: similarPayments } = await supabase
        .from("payments")
        .select("*")
        .ilike("cashSale_number", `%${cashSale.cashSale_number.slice(-4)}%`)
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      fetchStockItems()
      if (mode === "create") {
        generateCashSaleNumber().then(setOrderNumber)
        resetForm()
        setOrderDate(new Date().toISOString().split('T')[0])
      } else if (cashSale) {
        loadCashSaleData()
        fetchPaymentInfo() // Fetch payment info when viewing/editing cashSale
        if (cashSale.date_created) {
          setOrderDate(cashSale.date_created.split('T')[0])
        }
      }
    }
  ...
  const createNewItem = (category: "cabinet" | "worktop" | "accessories" | "appliances" | "wardrobes" | "tvunit"): CashSaleItem => {
    return {
  ...
  const loadCashSaleData = () => {
    if (!cashSale) return
    
    setOrderNumber(cashSale.cashSale_number || "")
    setSelectedClient(cashSale.client || null)
      setClientSearchTerm(cashSale.client?.name || "")
    setLabourPercentage(Number(cashSale.labour_percentage) || 30)
    setIncludeWorktop(cashSale.include_worktop || false)
      setIncludeAccessories(cashSale.include_accessories || false)
    setIncludeAppliances(cashSale.include_appliances || false)
    setIncludeWardrobes(cashSale.include_wardrobes || false)
    setIncludeTvUnit(cashSale.include_tvunit || false)
      setNotes(cashSale.notes || "")
      setTermsConditions(cashSale.terms_conditions || "")
    
    // Load custom section names if available
    if (cashSale.section_names) {
      setSectionNames(prev => ({
        ...prev,
        ...cashSale.section_names
      }))
    }
    
    // Load VAT percentage from database
    if (cashSale.vat_percentage) {
      setVatPercentage(cashSale.vat_percentage)
    }
    
    // Load discount amount from database
    if (cashSale.discount_amount) {
      setDiscountAmount(cashSale.discount_amount)
    }
    
    // Load items by category
    if (cashSale.items) {
      const cabinet = cashSale.items.filter((item: any) => item.category === "cabinet" && !item.description.includes("Labour Charge"));
      const worktop = cashSale.items.filter((item: any) => item.category === "worktop");
      const accessories = cashSale.items.filter((item: any) => item.category === "accessories");
      const appliances = cashSale.items.filter((item: any) => item.category === "appliances");
      const wardrobes = cashSale.items.filter((item: any) => item.category === "wardrobes");
      const tvunit = cashSale.items.filter((item: any) => item.category === "tvunit");
    ...
    setCabinetLabourPercentage(Number(cashSale.cabinet_labour_percentage) || 30)
    setAccessoriesLabourPercentage(Number(cashSale.accessories_labour_percentage) || 30)
          setAppliancesLabourPercentage(Number(cashSale.appliances_labour_percentage) || 30)
      setWardrobesLabourPercentage(Number(cashSale.wardrobes_labour_percentage) || 30)
      setTvUnitLabourPercentage(Number(cashSale.tvunit_labour_percentage) || 30)
    setWorktopLaborQty(cashSale.worktop_labor_qty ?? 1)
    setWorktopLaborUnitPrice(cashSale.worktop_labor_unit_price ?? 3000)
  }
  ...
  const updateItem = (category: "cabinet" | "worktop" | "accessories" | "appliances" | "wardrobes" | "tvunit", index: number, field: keyof CashSaleItem, value: any) => {
  ...
  const generatePDF = async () => {
    try {
      ...
      const { generateCashSalePDF } = await import('@/lib/pdf-template');
      ...
      const accessoriesLabour = includeAccessories ? (totals.accessoriesTotal * accessoriesLabourPercentage) / 100 : 0;
      ...
      // Add accessories section header and items (only if section is visible)
      ...
      const { template, inputs } = await generateCashSalePDF(cashSaleData);
      ...
      link.href = url;
      link.download = `cashSale-${orderNumber}.pdf`;
  ...
  const handleShare = async () => {
    try {
      ...
      if (canShareFile && pdfBlob) {
        const file = new File([pdfBlob], `cashSale-${orderNumber}.pdf`, { type: 'application/pdf' })
        // @ts-ignore - canShare exists in supporting browsers
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          // @ts-ignore - share with files
          await navigator.share({
            title: `Quotation ${orderNumber}`,
            text: `Quotation ${orderNumber} from Cabinet Master Styles`,
            files: [file]
          })
          setShowPrintModal(false)
          return
  ...
  const generatePDFForViewing = async () => {
    ...
    try {
      ...
      const { generate } = await import('@pdfme/generator');
      const { text, rectangle, line, image } = await import('@pdfme/schemas');
      const { generateCashSalePDF, imageToBase64 } = await import('@/lib/pdf-template');
      ...
      const items: any[] = [];
      const grouped = cashSale.items?.reduce((acc: Record<string, any[]>, item: any) => {
      ...
      sectionOrder.forEach((category) => {
        ...
        if (category === 'worktop' && cashSale.worktop_labor_qty && cashSale.worktop_labor_unit_price) {
          currentSectionTotal += cashSale.worktop_labor_qty * cashSale.worktop_labor_unit_price;
        }
        ...
        if (category !== 'worktop' && category !== 'cabinet' && itemsInCategory.length > 0) {
          ...
          let labourPercentage = cashSale.labour_percentage || 30; // Use general labour_percentage as default
          switch (category) {
            case 'cabinet':
              labourPercentage = cashSale.cabinet_labour_percentage || cashSale.labour_percentage || 30;
              break;
            case 'accessories':
              labourPercentage = cashSale.accessories_labour_percentage || cashSale.labour_percentage || 30;
              break;
            case 'appliances':
              labourPercentage = cashSale.appliances_labour_percentage || cashSale.labour_percentage || 30;
              break;
            case 'wardrobes':
              labourPercentage = cashSale.wardrobes_labour_percentage || cashSale.labour_percentage || 30;
              break;
            case 'tvunit':
              labourPercentage = cashSale.tvunit_labour_percentage || cashSale.labour_percentage || 30;
              break;
            default:
              labourPercentage = cashSale.labour_percentage || 30;
          }
        ...
        const sectionLabels: { [key: string]: string } = {
          cabinet: cashSale.section_names?.cabinet || "General",
          worktop: cashSale.section_names?.worktop || "Worktop", 
          accessories: cashSale.section_names?.accessories || "Accessories",
          appliances: cashSale.section_names?.appliances || "Appliances",
          wardrobes: cashSale.section_names?.wardrobes || "Wardrobes",
          tvunit: cashSale.section_names?.tvunit || "TV Unit"
        };
        ...
        // Add labour charge logic to cashSale modal PDF generation
        ...
        if (category === 'worktop' && cashSale.worktop_labor_qty && cashSale.worktop_labor_unit_price) {
          items.push({
            ...
            quantity: cashSale.worktop_labor_qty,
            unit: "per slab",
            description: "Worktop Installation Labor",
            unitPrice: cashSale.worktop_labor_unit_price.toFixed(2),
            total: (cashSale.worktop_labor_qty * cashSale.worktop_labor_unit_price).toFixed(2)
          });
          itemNumber++;
        }
        ...
        // Add worktop labor to section total if it exists
        if (category === 'worktop' && cashSale.worktop_labor_qty && cashSale.worktop_labor_unit_price) {
          sectionTotal2 += cashSale.worktop_labor_qty * cashSale.worktop_labor_unit_price;
        }
      ...
      // Prepare cashSale data (same as working download PDF)
      const { template, inputs } = await generateCashSalePDF({
        companyName: "CABINET MASTER STYLES & FINISHES",
        companyLocation: "Location: Ruiru Eastern By-Pass",
        companyPhone: "Tel: +254729554475",
        companyEmail: "Email: cabinetmasterstyles@gmail.com",
        clientNames: cashSale.client?.name || "",
        siteLocation: cashSale.client?.location || "",
        mobileNo: cashSale.client?.phone || "",
        date: new Date(cashSale.date_created).toLocaleDateString(),
        deliveryNoteNo: "Delivery Note No.",
        orderNumber: cashSale.cashSale_number,
        items,
        sectionTotals: [],
        total: cashSale.grand_total || 0,
        notes: cashSale.notes || "",
        terms: parseTermsAndConditions(cashSale.terms_conditions || ""),
        preparedBy: "",
        approvedBy: "",
  ...
  const handleSave = async () => {
    ...
    // Confirm cashSale number if it's a new cashSale
    // Removed localStorage logic, so this block is effectively removed.
    // The generateCashSaleNumber function now handles the number generation.

    await onSave(cashSaleData)
    onClose()
    } catch (error) {

      toast.error("Failed to save cashSale")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <>
      <div className="modal fade show d-block cashSale-modal" style={{ backgroundColor: "rgba(0,0,0,0.5)" }}>
        <div 
            ...
            <div className="d-flex align-items-center">
              ...
              <div>
                <h5 className="modal-title mb-1 fw-bold" style={{ color: "#ffffff" }}>
                  {mode === "create" ? "New Cash Sale" : mode === "edit" ? "Edit Cash Sale" : "View Cash Sale"}
            </h5>
                {mode !== "view" && (
                  <p className="mb-0 text-white small">Create a detailed cashSale for your client</p>
                )}
              </div>
            </div>
            <button
                ...
                <div className="card-body p-4">
                  ...
                  <div className="mb-3">
                    ...
                    {cabinetItems.map((item, index) => (
                      <div key={item.id} className="d-flex flex-column flex-md-row align-items-start align-items-md-center mb-3 mb-md-2 cashSale-item-row worktop-row">
                    ...
                    {/* Minimalistic Labour Footer for Cabinet Section */}
                      {mode !== "view" && (
                        <div className="d-flex align-items-center mt-2 p-2 cashSale-item-row worktop-row worktop-labor-row" style={{ background: "rgba(255,255,255,0.04)", borderRadius: "10px" }}>
                  ...
                  {includeWardrobes && (
                    <div className="mb-3">
                      ...
                      {wardrobesItems.map((item, index) => (
                        <div key={item.id} className="d-flex flex-md-row align-items-start align-items-md-center mb-3 mb-md-2 cashSale-item-row worktop-row" style={{ flexDirection: 'row' }}>
                  ...
                  {notesExpanded && (
                    <div className="px-3 pb-3 pt-0">
                      <textarea
                        ...
                        style={{ borderRadius: "10px", border: "1px solid rgba(233,236,239,0.6)", minHeight: "40px", resize: "none", overflow: "hidden", fontSize: "14px" }}
                        readOnly={isReadOnly}
                      />
                    </div>
                  )}
