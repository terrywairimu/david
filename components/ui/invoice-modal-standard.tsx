...
const generateInvoiceNumber = async () => {
  try {
    ...
    // Query Supabase for the latest invoice number for this year/month
    const { data, error } = await supabase
      .from("invoices")
      .select("invoice_number")
      .ilike("invoice_number", `QT${year}${month}%`)
      .order("invoice_number", { ascending: false })
      .limit(1)
    if (error) throw error
    let nextNumber = 1
    if (data && data.length > 0) {
      const match = data[0].invoice_number.match(/QT\d{4}(\d{3})/)
      if (match) {
...
const InvoiceModal: React.FC<InvoiceModalProps> = ({
  ...
  const fetchPaymentInfo = async () => {
    ...
    try {
      // First, check all payments for this invoice regardless of status - check both fields
      const { data: allPayments } = await supabase
        .from("payments")
        .select("*")
        .or(`invoice_number.eq.${invoice.invoice_number},paid_to.eq.${invoice.invoice_number}`)

      // Then get only completed payments - check both invoice_number and paid_to fields
      ...
      const paymentPercentageValue = invoice.grand_total > 0 ? (totalPaidAmount / invoice.grand_total) * 100 : 0
      ...
      // Also check if there are payments with different invoice_number formats
      const { data: similarPayments } = await supabase
        .from("payments")
        .select("*")
        .ilike("invoice_number", `%${invoice.invoice_number.slice(-4)}%`)
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      fetchStockItems()
      if (mode === "create") {
        generateInvoiceNumber().then(setOrderNumber)
        resetForm()
        setOrderDate(new Date().toISOString().split('T')[0])
      } else if (invoice) {
        loadInvoiceData()
        fetchPaymentInfo() // Fetch payment info when viewing/editing invoice
        if (invoice.date_created) {
          setOrderDate(invoice.date_created.split('T')[0])
        }
      }
    }
  ...
  const loadInvoiceData = () => {
    ...
    setOrderNumber(invoice.invoice_number || "")
    setSelectedClient(invoice.client || null)
      setClientSearchTerm(invoice.client?.name || "")
    setLabourPercentage(Number(invoice.labour_percentage) || 30)
    setIncludeWorktop(invoice.include_worktop || false)
      setIncludeAccessories(invoice.include_accessories || false)
    ...
    if (invoice.vat_percentage) {
      setVatPercentage(invoice.vat_percentage)
    }
    ...
    setCabinetLabourPercentage(Number(invoice.cabinet_labour_percentage) || 30)
    setAccessoriesLabourPercentage(Number(invoice.accessories_labour_percentage) || 30)
          setAppliancesLabourPercentage(Number(invoice.appliances_labour_percentage) || 30)
      setWardrobesLabourPercentage(Number(invoice.wardrobes_labour_percentage) || 30)
      setTvUnitLabourPercentage(Number(invoice.tvunit_labour_percentage) || 30)
    setWorktopLaborQty(invoice.worktop_labor_qty ?? 1)
    setWorktopLaborUnitPrice(invoice.worktop_labor_unit_price ?? 3000)
  }
  ...
  const generatePDF = async () => {
    try {
      ...
      const { generateInvoicePDF } = await import('@/lib/pdf-template');
      ...
      const { template, inputs } = await generateInvoicePDF(invoiceData);
  ...
  const handleShare = async () => {
    try {
      ...
      if (canShareFile && pdfBlob) {
        const file = new File([pdfBlob], `invoice-${orderNumber}.pdf`, { type: 'application/pdf' })
  ...
  const generatePDFForViewing = async () => {
    ...
    try {
      ...
      const { generate } = await import('@pdfme/generator');
      const { text, rectangle, line, image } = await import('@pdfme/schemas');
      const { generateInvoicePDF, imageToBase64 } = await import('@/lib/pdf-template');
      ...
      sectionOrder.forEach((category) => {
        ...
        if (category === 'worktop' && invoice.worktop_labor_qty && invoice.worktop_labor_unit_price) {
          currentSectionTotal += invoice.worktop_labor_qty * invoice.worktop_labor_unit_price;
        }
        ...
        // Add labour charge logic to invoice modal PDF generation
        ...
        // Add worktop labor to section total if it exists
        if (category === 'worktop' && invoice.worktop_labor_qty && invoice.worktop_labor_unit_price) {
          sectionTotal2 += invoice.worktop_labor_qty * invoice.worktop_labor_unit_price;
        }
      ...
      const { template, inputs } = await generateInvoicePDF({
        companyName: "CABINET MASTER STYLES & FINISHES",
        companyLocation: "Location: Ruiru Eastern By-Pass",
        companyPhone: "Tel: +254729554475",
        companyEmail: "Email: cabinetmasterstyles@gmail.com",
  ...
  const handleSave = async () => {
    ...
    // Confirm invoice number if it's a new invoice
    // Removed localStorage logic, so this block is effectively removed.
    // The generateInvoiceNumber function now handles the number generation.
  ...
  return (
    <>
      <div className="modal fade show d-block invoice-modal" style={{ backgroundColor: "rgba(0,0,0,0.5)" }}>
        <div 
          ...
          <div className="modal-header border-0" style={{ padding: "24px 32px 16px" }}>
            <div className="d-flex align-items-center">
              ...
              <div>
                ...
                {mode !== "view" && (
                  <p className="mb-0 text-white small">Create a detailed invoice for your client</p>
                )}
              </div>
            </div>
            <button
                ...
                <div className="card-body p-4">
                  ...
                  <div className="mb-3">
                    ...
                    {cabinetItems.map((item, index) => (
                      <div key={item.id} className="d-flex flex-column flex-md-row align-items-start align-items-md-center mb-3 mb-md-2 invoice-item-row worktop-row">
                    ...
                    {/* Minimalistic Labour Footer for Cabinet Section */}
                      {mode !== "view" && (
                        <div className="d-flex align-items-center mt-2 p-2 invoice-item-row worktop-row worktop-labor-row" style={{ background: "rgba(255,255,255,0.04)", borderRadius: "10px" }}>
                  ...
                  {includeWardrobes && (
                    <div className="mb-3">
                      ...
                      {wardrobesItems.map((item, index) => (
                        <div key={item.id} className="d-flex flex-md-row align-items-start align-items-md-center mb-3 mb-md-2 invoice-item-row worktop-row" style={{ flexDirection: 'row' }}>
            ...
            {mode === "view" ? (
              <>
                ...
                <div className="d-flex d-md-none flex-column w-100 gap-2">
                  {(() => {
                    const showButton = hasPayments && invoice?.status !== "converted_to_cash_sale" && onProceedToCashSale;
                    return showButton ? (
                      <div className="d-flex justify-content-end">
                        <button
                          type="button"
                          className="btn btn-primary"
                          onClick={() => onProceedToCashSale && onProceedToCashSale(invoice)}
                          style={{ 
                            borderRadius: "12px", 
                            padding: "10px 24px",
